...
public final class VRLXAbstractSession {
    ...
    static {
        ...
        getVRLXFormat().addTask(CONTENT_PATH,
                new AbstractSessionTask());
    }

    // no instanciation allowed
    private VRLXAbstractSession() {
    ...
    public static IOModel getIOModel() {
        return new DefaultIOModel(new XMLEntryFactory());
    }


    public static class AbstractSessionTask implements SessionTask {
        ...
        public boolean load(Canvas canvas, SessionEntry entry) {
            ...
            if (session == null) {
                VRLXSessionController.showErrorMessage(
                        canvas, null,
                        "Error while loading AbstractSession:");
                return false;
            }
            ...
            vCanvas.setSession(session);
            
            return true;
        }
        
        public AbstractSession load(SessionEntry entry) throws IOException {
            AbstractSession session = null;
            ...
            if (session == null) {
                throw new IOException("Error while loading AbstractSession:");
            }
        ...
        public boolean save(SessionEntry entry, AbstractSession session) {
