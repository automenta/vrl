 ...
 */
@TypeInfo(type=String.class, input = true, output = true, style="method-name")
public class MethodNameType extends StringType {

    public MethodNameType() {
        setValueName("Method-Name:");
    }
    ...
    protected void evaluateContract() {
        ...
        if (!VLangUtils.isClassNameValid(s)) {
            getMainCanvas().getMessageBox().addMessage("Method-Name invalid:",
                    ">> the method-name you entered is invalid. "
                    + "Only names that match the following regular expression"
                    + " are allowed: [a-zA-Z$_][a-zA-Z$_0-9]*",
